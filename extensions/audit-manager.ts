...
declare function complete(model: any, params: { messages: any[] }, options?: { apiKey?: string; headers?: Record<string, string>; maxTokens?: number }): Promise<string>;
...
export class AuditManager {
	...
	public async dispatchAuditAgents(files: string[]): Promise<string[]> {
		...
		console.log(
			`[pi-audit-master] Dispatching ${personas.length} agents to audit ${files.length} files...`,
		);
		...
		for (let i = 0; i < results.length; i++) {
			...
			if (result.status === "fulfilled") {
				...
				console.log(`[pi-audit-master] ${persona} completed successfully`);
			} else {
				console.error(`[pi-audit-master] ${persona} failed: ${result.reason}`);
	 ...
	 */
	private async readFilesForAudit(files: string[]): Promise<Map<string, string>> {
		...
		for (const file of files) {
			try {
				...
				console.warn(`[pi-audit-master] Cannot read ${file}: ${(err as Error).message}`);
			}
		}
	...
	private async runAuditAgent(
		...
		try {
			...
			console.warn(`[pi-audit-master] AI completion failed for ${persona}, using static analysis`);
		...
		for (const [file, content] of fileContents) {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (persona === "Performance Oracle") {
					if (line.includes("for") && lines.slice(i, i + 10).join("").includes("for")) {
				...
				if (persona === "Quality Guardian") {
					if (line.match(/\b(magic|hardcoded)\b/i) || line.match(/\b\d{3,}\b/) && !line.includes("//")) {
